...
def generate_coefficients():
    ...
    interval [-0.5, 0.5).
    """
    sequence = [random.uniform(-0.5,0.5) for i in range(5)]
    return sequence
...
def generate_data(m, coefficients):
    ...
    """
    # Generate evenly spaced X values from -5 to 5
    X = np.linspace(-5, 5, m).reshape(m, 1)

    # Compute y values using the polynomial formula with noise
    a, b, c, d, e = coefficients
    y = (a + b * X + c * X**2 + d * X**3 + e * X**4 +
         np.random.normal(0, 1, (m, 1)))
...
def fit_curve(X, y, degree):
    ...
    model = (lin_reg.intercept_, lin_reg.coef_)
    return model
...
def plot_curve(degree, model):
    ...
    # Calculate predictions: intercept + coefficients * features
    ys = model[0] + X_poly @ model[1].T

    plt.plot(X, ys, label=f'Degree {degree}')
...
def mse(X, y, degree, model):
    ...
    # Calculate predictions: intercept + coefficients * features
    y_pred = model[0] + X_poly @ model[1].T
